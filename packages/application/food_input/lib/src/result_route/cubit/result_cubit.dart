...
class ResultCubit extends Cubit<ResultState> {
  ResultCubit({required this.foodInputRepository}) : super(ResultState()) {
    _subscription = foodInputRepository.foodList.listen((foodList) {
      _enhancedEmit(state.copyWith(foods: foodList));
    });
  }
  ...
  void updateFood(Food food) async {
    _enhancedEmit(
      state.copyWith(updatingStatus: AsyncProcessingStatus.loading),
    );
    try {
      await foodInputRepository.updateFoodsNutritions(food);
      _enhancedEmit(
        state.copyWith(updatingStatus: AsyncProcessingStatus.success),
      );
  ...
  void deleteFood(List<String> foodIds) async {
    _enhancedEmit(
      state.copyWith(deletingStatus: AsyncProcessingStatus.loading),
    );
    try {
      await foodInputRepository.deleteFoodsNutritions(foodIds);
      _enhancedEmit(
        state.copyWith(deletingStatus: AsyncProcessingStatus.success),
      );
    } on InternetConnectionException {
      _enhancedEmit(
        state.copyWith(
  ...
  void _enhancedEmit(ResultState state) {
    if (!isClosed) {
      emit(state);
    }
  }
