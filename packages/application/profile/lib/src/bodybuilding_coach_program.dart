          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              if (index == 0) {
                ...
                return ProgramCard(program: state.coachPrograms[index - 1]);
              }
            },
            separatorBuilder: (context, index) {
              return SizedBox(width: context.sizeExtenstion.medium);
            },
            itemCount: state.coachPrograms.length + 1,
          ),
        );
      },
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Form(
      ...
      child: AppDialog(
        ...
        submitButton: BlocConsumer<ProfileCubit, ProfileState>(
          ...
          listener: (context, state) {
            if (state.updatingCoachProgram.isConnectionError) {
              ...
              context.read<ProfileCubit>().readCoachPrograms();
            }
          },
          buildWhen: (previous, current) =>
              previous.updatingCoachProgram != current.updatingCoachProgram,
          builder: (context, state) {
            final buttonLabel = widget.coachProgram == null
                ? context.l10n.profileCoachProfileCoachProgramSaveButtonLabel
                : context.l10n.update;
                ...
                : AppTextButton(
                    ...
                    onTap: () {
                      if (_formKey.currentState!.validate()) {
                        context.read<ProfileCubit>().onChangeCoachProgram(
                          _coachProgram,
                        );
                        context.read<ProfileCubit>().upsertCoachProgram();
                      }
                    },
                  );
          },
        ...
        builder: (context, state) {
          return state.deletingCoachProgram.isLoading
              ...
              : AppTextButton(
                  ...
                  onTap: () {
                    context.read<ProfileCubit>().deleteCoachProgram(id);
                  },
                );
        },
      ),
